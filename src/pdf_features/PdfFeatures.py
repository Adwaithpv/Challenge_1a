...
class PdfFeatures(BaseModel):
    ...
    def is_pdf_encrypted(pdf_path):
        try:
            ...
            # qpdf not available, assume PDF is not encrypted
            return False
    ...
    def from_pdf_path(pdf_path, xml_path: str | Path = None):
        ...
        if PdfFeatures.is_pdf_encrypted(pdf_path):
            try:
                ...
                # qpdf not available, skip decryption
                pass

        # Try full path to pdftohtml first, then system PATH
        pdftohtml_path = r"C:\Users\adwai\AppData\Local\Microsoft\WinGet\Packages\oschwartz10612.Poppler_Microsoft.Winget.Source_8wekyb3d8bbwe\poppler-24.08.0\Library\bin\pdftohtml.exe"
        
        try:
            if Path(pdftohtml_path).exists():
                subprocess.run([pdftohtml_path, "-nodrm", "-i", "-xml", "-zoom", "1.0", pdf_path, xml_path])
            else:
                subprocess.run(["pdftohtml", "-nodrm", "-i", "-xml", "-zoom", "1.0", pdf_path, xml_path])
        except FileNotFoundError:
            raise FileNotFoundError("pdftohtml not found. Please install poppler-utils.")

        if not PdfFeatures.contains_text(xml_path):
            try:
                if Path(pdftohtml_path).exists():
                    subprocess.run([pdftohtml_path, "-nodrm", "-i", "-hidden", "-xml", "-zoom", "1.0", pdf_path, xml_path])
                else:
